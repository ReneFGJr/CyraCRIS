...
$routes->post('ppg/(:num)/logo', 'Ppg::uploadLogo/$1', ['filter' => 'csrf']);
$routes->post('ppg/(:num)/linhas/(:num)/docentes', 'Ppg::adicionarDocente/$1/$2', ['filter' => 'csrf']);
$routes->get('person/search', 'Docent::search');
$routes->get('person/(:num)', 'Docent::show/$1');
$routes->get('person/(:num)/foto', 'Docent::foto/$1');
...
$routes->post('person/(:num)/foto', 'Docent::uploadFoto/$1', ['filter' => 'csrf']);
$routes->post('person/(:num)/foto-lattes', 'Docent::extrairFotoLattes/$1', ['filter' => 'csrf']);
$routes->post('person/(:num)/rdf-data', 'Docent::adicionarRdfData/$1', ['filter' => 'csrf']);
